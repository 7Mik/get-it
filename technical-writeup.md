...
The decision to drive every agent through the **user's own ChatGPT login over the official Codex CLI** is not a cost-cutting move — it is the architectural choice that shapes the entire product. There is no server-side OpenAI key, no shared pool of credits, and no app-side metering of model usage. The Electron shell bundles the Codex CLI binary per platform/arch, the first-launch wizard spawns `codex login` so the user authenticates against OpenAI directly, and every subsequent `codex exec` call runs against that account at whatever tier the user pays for. The app sees the same auth state Codex itself sees: a successful login, a rate-limit window, an expired token. Nothing more.

Three things follow from that:

1. **No second subscription, ever.** Every other AI-study tool we benchmarked layers a marked-up monthly fee on top of an API key the vendor holds — the student ends up paying twice for the same model access. Get It. cannot do that because we never hold the key in the first place. ChatGPT Plus is the practical floor for sustained study sessions (the free tier signs in but its Codex allowance is intentionally small), and everything above Plus — Pro, Team, Enterprise, Edu — just gives the same flow more headroom, no app-side switch. This is the single sentence that explains the product positioning: *one subscription, the one you already have, no markup.*

2. **No data resale and no transit-stage intermediary.** Because we never proxy the model traffic through our infrastructure, there is no Get It. infrastructure for that traffic to flow through. The Codex CLI talks to OpenAI on the user's machine; we observe nothing. The work-context journal, the knowledge graph, the per-doc folders all live under an OS-native user-data directory on local disk. There is no upload step, no opt-in cloud sync, no analytics SDK. We sometimes describe this as "downloadable in one click" because the same JSON file the evaluator reads is the one the user can pull out from the right-pane menu — but the more honest framing is that there is nothing else *to* download: this is the only state, and it is already on the user's computer.

3. **The transport is replaceable.** Codex CLI is one of several ways the app could speak to a model. Today it is the one we ship because it has by far the best ergonomics around per-tier login, the bundled binary is small, the official SDK gives us schema-typed responses without DIY JSON-Schema enforcement, and it is the only path through which a ChatGPT Plus account can drive a developer-facing CLI without an extra API-key purchase. If OpenAI deprecates Codex, or if a comparable bring-your-own-account transport for another provider emerges, the `runJson` helper in `lib/codex.ts` is the only place that has to change.

The same property protects the project legally. **Get It. is not affiliated with OpenAI, not endorsed by OpenAI, not sponsored by OpenAI**, and not a derivative work of any closed-source OpenAI software — it is an independent application that interoperates with the publicly released Codex CLI and uses the end user's own credentials. We are careful to say so explicitly: in the README's Notice section, in the in-app first-launch wizard, and again at the bottom of this document. The user's actual use of OpenAI's models through Get It. is governed by OpenAI's own Terms of Use, Usage Policies, and Privacy Policy — those documents are authoritative for what is and isn't allowed, what happens to the data on OpenAI's side, and what each subscription tier covers. We have no insight into that loop and no way to override it.

The flip side is honesty about what the user is paying when they upgrade their ChatGPT tier: they are paying OpenAI for model access; they are not paying us. This is the inverse of how almost every AI-wrapper product is priced. We treat it as a feature.
...
The user's use of OpenAI's models through Get It. is subject to OpenAI's own [Terms of Use](https://openai.com/policies/terms-of-use), [Usage Policies](https://openai.com/policies/usage-policies), and [Privacy Policy](https://openai.com/policies/privacy-policy), and to the Codex CLI's [own license and release notes](https://github.com/openai/codex). Those documents are the authoritative source for what the underlying model service permits and how data is handled on OpenAI's side.

Source code for Get It. is licensed under the **Apache License, Version 2.0** — see [`LICENSE`](LICENSE) in the repository root.
